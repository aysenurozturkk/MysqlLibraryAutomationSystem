...
namespace Kütüphane_Otomasyonu
{
    public partial class KitapListelefrm : Form
    {
        ...
        {
            daset.Tables["kitap"].Clear();//üst üste gelmesin diye öncesinde temizler
            baglanti.Open();
            SqlDataAdapter adtr = new SqlDataAdapter("select *from kitap where barkodno like '%" + txtBarkodAra.Text + "%' ", baglanti); // yüzdenin anlamı tcyi bulunca önde arkada sında bize gösterecek
            adtr.Fill(daset, "kitap");
            dataGridView1.DataSource = daset.Tables["kitap"];
            baglanti.Close();
        }
